...
const validateEmail = () => {
  coldTime.value = 60
  let handle;
  get(`/api/auth/ask-code?email=${emailForm.email}&type=reset`, () => {
...
<template>
  <div style="display: flex;gap: 10px">
    ...
    <div class="info-card" style="flex: 50%">
      <div class="title"><i class="fa-solid fa-users"></i> 子管理账户管理</div>
      <el-divider style="margin: 10px 0"/>
      ...
      <div v-else>
        <el-empty :image-size="100" description="还没有任何管理账户哦" v-if=true>
          <el-button :icon="Plus" type="primary" plain
                     @click="createAccount = true" v-if=store.isAdmin>添加子管理账户</el-button>
        </el-empty>
        <el-empty :image-size="100" description="子管理账户只能由管理员账号进行操作" v-else/>
      </div>
    </div>
